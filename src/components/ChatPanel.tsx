...
import {
  ArrowLeft, Send, FileText, BookOpen, Search,
  Trash2, Clock, Layers, Bot, Copy, Check, Download,
  Menu, X
} from 'lucide-react';
...
export default function ChatPanel({ doc, onBack, otherDocs, onSelectDoc }: ChatPanelProps) {
  ...
  const handleSendMessage = (textToSend: string) => {
    ...
    const userMsg: Message = { id: `user-${Date.now()}`, sender: 'user', text: textToSend, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
    ...
    setTimeout(() => {
      ...
      if (lowercaseQuery.includes('summar') || lowercaseQuery.includes('insight')) {
        ...
        responseText = `### Critical Metrics & Performance Data\n\nI found the following quantitative data and highlights in **${doc.name}**:\n\n* **Work speed (throughput)**: Boosted by **42%** during peak tests.\n* **Timing consistency (standard deviation)**: Remains stable under **1.2 milliseconds** overall.\n* **Trial samples**: Documented over **14 separate verification rounds** with positive results.`;
      } else {
        responseText = `I have read through the pages of **${doc.name}** to look for your query: *"${textToSend}"*.\n\nAccording to the details found in the document, the system ensures proper operation by isolating active environments. This allows high-speed safety and reliable behavior during usage.`;
      }

      const botMsg: Message = { id: `assistant-${Date.now()}`, sender: 'assistant', text: responseText, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) };
  ...
  const handleClearHistory = () => {
    if (window.confirm("Are you sure you want to clear your conversation history?")) {
      setMessages([{ id: 'welcome', sender: 'assistant', text: `Chat history cleared! Ask me anything about **${doc.name}**.`, timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
    }
  };
          ...
          <>
            ...
            >
              <div className="p-4 border-b border-slate-200 flex items-center justify-between">
                <button onClick={onBack} className="flex items-center space-x-2 text-slate-600 font-semibold text-sm transition-colors">
                  <div className="p-1.5 rounded-md transition-colors">
                    <ArrowLeft size={16} />
                  </div>
                  <span>Dashboard</span>
                </button>
                <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-1.5 text-slate-400 rounded-md">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="p-5 border-b border-slate-200 bg-slate-50">
                <div className="flex items-start space-x-3 mb-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-sm font-bold text-slate-900 line-clamp-2 leading-tight" title={doc.name}>{doc.name}</h3>
                    <p className="text-[11px] text-slate-500 font-mono mt-0.5">ID: {doc.id.slice(0, 8)}</p>
                  </div>
                </div>

                <div className="space-y-2 bg-white border border-slate-200 rounded-lg p-3 text-xs font-medium text-slate-700">
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 flex items-center"><Layers size={14} className="mr-1.5" /> Pages</span>
                    <span>{doc.pageCount}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-500 flex items-center"><BookOpen size={14} className="mr-1.5" /> Size</span>
                    <span>{doc.size}</span>
                  </div>
                  <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                    <span className="text-slate-500">Status</span>
                    <span className="text-cyan-600 font-semibold flex items-center">
                      <span className="w-1.5 h-1.5 rounded-full bg-cyan-500 mr-1.5" /> Analyzed
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                <div>
                  <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Other Documents</h4>
                  <div className="space-y-1">
                    {otherDocs.filter(d => d.id !== doc.id).length === 0 ? (
                      <div className="text-xs text-slate-400 bg-slate-50 p-3 rounded-lg text-center border border-dashed border-slate-200">No other PDFs found</div>
                    ) : (
                      otherDocs.filter((d) => d.id !== doc.id).map((d) => (
                        <button
                          key={d.id} onClick={() => { onSelectDoc(d); setIsSidebarOpen(false); }}
                        ...
                        >
                          <div className="p-1.5 bg-slate-100 rounded-md group-">
                            <FileText size={14} className="text-slate-500 group-" />
                          </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-white">
        ...
        <div className="px-5 py-3 lg:py-4 bg-white border-b border-slate-200 flex items-center justify-between z-10">
          <div className="flex items-center space-x-3">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 -ml-2 text-slate-600 rounded-md">
              <Menu size={20} />
            </button>
          ...
          <div className="flex items-center space-x-1">
            <button onClick={handleDownloadTranscript} className="text-slate-500 p-1.5 lg:p-2 rounded-md transition-colors" title="Download Transcript"><Download size={18} /></button>
            <div className="w-px h-4 bg-slate-200 mx-1" />
            <button onClick={handleClearHistory} className="text-slate-500 p-1.5 lg:p-2 rounded-md transition-colors" title="Clear Chat"><Trash2 size={18} /></button>
          </div>
        </div>
 ...
 : 'bg-white text-slate-800 rounded-2xl rounded-tl-sm border border-slate-200'
                        ...
                        <div className="space-y-3">
                          {msg.text.split('\n\n').map((paragraph, pIdx) => {
                            if (paragraph.startsWith('###')) return <h4 key={pIdx} className="font-bold text-slate-900 text-base mb-1">{paragraph.replace('###', '').trim()}</h4>;
                            if (paragraph.startsWith('*') || paragraph.startsWith('-')) {
                              return (
                                <ul key={pIdx} className="list-disc pl-5 space-y-1 my-2">
                                  {paragraph.split('\n').map((line, lIdx) => (
                                    <li key={lIdx}>{line.replace(/^[\s*-]+/, '').trim().split('**').map((chunk, cIdx) => {
                                      if (cIdx % 2 === 1) return <strong key={cIdx} className={msg.sender === 'user' ? 'text-white font-bold' : 'text-slate-900 font-bold'}>{chunk}</strong>;
                                      return chunk;
                                    })}</li>
                                  ))}
                                </ul>
                            ...
                            return (
                              <p key={pIdx} className={pIdx > 0 ? 'mt-3' : ''}>
                                {paragraph.split('**').map((chunk, chunkIdx) => {
                                  if (chunkIdx % 2 === 1) return <strong key={chunkIdx} className={msg.sender === 'user' ? 'text-white' : 'text-slate-900 font-semibold'}>{chunk}</strong>;
                                  return chunk;
                                })}
                              </p>
                            );
                        ...
                        {msg.sender === 'assistant' && (
                          <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-end">
                            <button onClick={() => handleCopyText(msg.text, msg.id)} className="text-slate-400 flex items-center space-x-1 text-xs font-medium bg-slate-50 px-2 py-1 rounded-md transition-colors">
                              {copiedId === msg.id ? <><Check size={14} className="text-emerald-500" /><span>Copied!</span></> : <><Copy size={14} /><span>Copy</span></>}
                            </button>
                          </div>
                        )}
                      </div>
                      <div className={`text-[11px] text-slate-400 mt-1.5 px-1 flex items-center space-x-1 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              {isTyping && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex space-x-3 max-w-4xl mx-auto w-full">
            ...
            {messages.length === 1 && (
              <div className="flex flex-wrap gap-2">
                {sampleSuggestions.map((suggestion, index) => (
                  <button key={index} onClick={() => handleSendMessage(suggestion.query)} className="text-xs lg:text-sm bg-white border border-slate-200 text-slate-600 px-3 py-1.5 lg:px-4 lg:py-2 rounded-lg transition-colors font-medium">
                    {suggestion.text}
                  </button>
                ))}
              </div>
